      ...
      <div class="col-md-4" v-for="(show, index) in shows" :key="index">
        <div class="card h-100">
          ...
          <div class="card-body">
            ...
            <b-button :href="'/comics/' + show.id" variant="primary"
              >See Comics Deatils</b-button
            >
          </div>
        </div>
      </div>
...
import { comicsUrl } from "../assets/api";
export default {
  name: "ComicsList",
  data() {
    return {
  ...
  async created() {
    ...
    try {
      const {
        data: { data },
      } = await axios.get(`${comicsUrl}&offset=${offset}&limit=15`);
