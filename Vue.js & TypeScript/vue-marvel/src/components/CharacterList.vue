      ...
      <div class="col-md-4" v-for="(show, index) in shows" :key="index">
        <div class="card h-100">
          ...
          <div class="card-body">
            ...
            <b-button :href="'/characters/' + show.id" variant="primary"
              >See Character Deatils
            </b-button>
          </div>
        </div>
      </div>
...
import { characterUrl } from "../assets/api";
export default {
  name: "CharacterList",
  data() {
    return {
  ...
  async created() {
    ...
    try {
      const {
        data: { data },
      } = await axios.get(`${characterUrl}&offset=${offset}&limit=15`);
